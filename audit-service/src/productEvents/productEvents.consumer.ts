...
export class ProductEventsConsumer {
    ...
    private readonly logger: JsonLogger = 
        LoggerFactory.createLogger(ProductEventsConsumer.name)

    constructor(
        private productEventsService: ProductEventsService
    ) {
        this.consumer = Consumer.create({
            queueUrl: process.env.AWS_SQS_QUEUE_PRODUCT_EVENTS_URL,
            waitTimeSeconds: 5,
            pollingWaitTimeMs: 0,
            handleMessage: async (message) => this.handleMessage(message)
        })

        this.logger.info('Starting AWS SQS product events consumer')
        this.consumer.start()
    }
        ...
        ns.run(async () => {
            ...
            switch(eventType) {
                ...
                case ProductEventType.PRODUCT_DELETED: {
                    const productEvent = JSON.parse(body.Message) as ProductEventDto
                    
                    await this.productEventsService.createProductEvent(
                    ...
                    this.logger.info(
                        ...
                        `Product event received: ${eventType}`
                    )            
                    break
                }
                default: {
